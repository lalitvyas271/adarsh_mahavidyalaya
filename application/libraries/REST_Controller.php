...
abstract class REST_Controller extends CI_Controller {
     ...
     * @var string
     */
    public $request_from = 'jangirsamaj';
    ...
    protected function _prepare_basic_auth() {
        ...
        elseif ($this->input->server('HTTP_AUTHENTICATION')) {
            if (strpos(strtolower($this->input->server('HTTP_AUTHENTICATION')), 'basic') === 0) {
                list($username, $password) = explode(':', base64_decode(substr($this->input->server('HTTP_AUTHORIZATION'),0)));
            }
        }
    ...
    public function validate_token(){
        ...
        if($token){
            session_write_close();
            session_id($this->encrypt->decode($token));
            session_start();
        }else{
            $this->response(array('status' => RES_CODE_SESS_OUT, 'msg' => 'You session hasbeen expired. (Token expired.)'), 404);
        }
        $this->load->library('kc_auth'); //LOAD THIS LIBRARY AFTER SESSION START 
    }
}
